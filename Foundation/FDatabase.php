...
class FDatabase
{
     ...
     */
    function cerca(string $key, string $value, string $str) 
    {
        $sql = '';
        $className = 'F'.$key;
        ...
        if($sql)
            return $this->exeCerca($classname, $value, $str, $sql);
        else return NULL;
    }
    ...
    /*function cerca(string $cont, string $str) 
    {
        $sql = '';
        if($cont == 0){
            ...
            $method= 'cercaCampagnaByUsername';
            $sql = FCampagna::$method(); 
            $className='FCampagna';
            $value='username';
        }
